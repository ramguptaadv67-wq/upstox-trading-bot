...
e.innerHTML='<option value="">— Select —</option>';r.expiries.forEach(exp=>e.innerHTML+='<option value="'+exp+'">'+exp+'</option>');e.disabled=false;updateJSON();}
obState.instrumentToken=r.instrument_key;document.getElementById('atmInfo').classList.remove('hidden');document.getElementById('atmStrike').textContent=r.atm_strike;document.getElementById('atmSpot').textContent=r.spot_price;document.getElementById('atmLtp').textContent=r.ltp||'—';updateJSON();}
...
loadStatus();loadInstruments().then(()=>{loadTradingConfig();document.querySelector('.tab.active')&&document.querySelector('.tab.active').click();});
